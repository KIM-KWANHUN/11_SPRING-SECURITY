    ...
    *   보안에 가장 빠르고 안전하다. */
    ...
    public SecurityFilterChain chain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(auth -> {
            ...
            // 그 외 어떠한 요청들은 권한 상관 없이 들어갈 수 있다. (단, 로그인 된 인원에 한해서)
            auth.anyRequest().authenticated();
        }).formLogin(login -> {
            login.loginPage("/auth/login"); // 로그인을 페이지 url 을 기술
            login.usernameParameter("user"); // 사용자가 ID 를 입력하는 필드(아이디 : <input type="text" name="user"><br>)
                                             // 에 있는 name 과 일치 해야 한다.
            login.passwordParameter("pass");
            // 사용자가 로그인에 성공 했을 시 보내줄 URL 기술
            login.defaultSuccessUrl("/",true);
            // 로그인에 실패 했을 시 보내줄 내용을 기술한 객체
            ...
            // 로그아웃 성공 시 요청 URL 기술 (메인페이지로 가게 해놧음 "/")
            logout.logoutSuccessUrl("/");
        }).sessionManagement(session -> {
            session.maximumSessions(1); // session 의 허용 갯수 제한, 한 사용자가 여러 창을 띄워 동시에 세션 여러 개 활성화 방지
